...
interface EditEmployeeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdateEmployee: (employeeData: any) => Promise<void>;
...
const EditEmployeeModal: React.FC<EditEmployeeModalProps> = ({ isOpen, onClose, onUpdateEmployee, employee }) => {
  ...
  // Pré-remplir le formulaire avec les données employé
  useEffect(() => {
    if (employee && isOpen) {
      setFormData({
  ...
  const validateForm = () => {
    ...
    if (formData.salary && isNaN(Number(formData.salary))) {
      newErrors.salary = 'Le salaire doit être un nombre';
    }
    if (formData.performance && (isNaN(Number(formData.performance)) || Number(formData.performance) < 0 || Number(formData.performance) > 5)) {
      newErrors.performance = 'La performance doit être entre 0 et 5';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const employeeData = {
        ...
        hireDate: formData.hireDate || employee.hireDate,
        isActive: formData.isActive,
        role: employee.role // Conserver le rôle original
      };
      ...
      setErrors({ general: 'Erreur lors de la modification de l\'employé' });
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  max="5"
                  step="0.1"
                  className={`input-field ${errors.performance ? 'border-red-300' : ''}`}
                  placeholder="Ex: 4.2"
                />
                {errors.performance && (
                  <p className="mt-1 text-sm text-red-600">{errors.performance}</p>
                )}
              </div>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Les employés inactifs n'apparaissent plus dans les listes principales.
            </p>
          </div>
